...
export function EvaluationDetailCompact({
  ...
  const listGridClass =
    'mt-2 grid grid-cols-1 gap-x-6 gap-y-1 text-sm text-atlas-muted sm:grid-cols-2';
  ...
  return (
    <article className="rounded-atlas-card border border-atlas-border bg-atlas-deep/80 p-4 shadow-atlas-card sm:p-6">
      ...
      <CompactBlock>
        ...
        <p className="mt-1 text-xs text-atlas-dim">
          Truth (ETS) out of {getScaleMax('ETS')}; Engagement (SES) and
          Integration (EIS) out of {getScaleMax('SES')}.
        </p>
